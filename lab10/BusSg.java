...
class BusSg {
  ...
  public static CompletableFuture<BusRoutes> findBusServicesBetween(BusStop stop, String searchString) {
      try {    
          return getBusServices(stop).thenApplyAsync(x -> x.stream())
             .thenApply(x -> x.collect(Collectors.toMap(
              service -> service, 
              service -> service.findStopsWith(searchString)))).thenApply(x -> { 
      return new BusRoutes(stop, searchString, x);
              });
    } catch (CompletionException e) {
      System.err.println("Unable to complete query: " + e);
      BusRoutes temp = new BusRoutes(stop, searchString, Map.of());
